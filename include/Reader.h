...
public:
    ...
    void readPosesFromJSON(const std::string& filename, std::vector<cv::Mat>& robot_poses, std::vector<cv::Mat>& camera_poses);
    void readPointsFromJSON(const std::string& file_path, std::vector<cv::Point3f>& poses3d_world, std::vector<std::vector<cv::Point2f>>& poses2d, std::vector<CameraParams>& camera_params);
